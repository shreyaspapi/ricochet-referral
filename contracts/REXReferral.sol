...
contract REXReferral is Ownable {
    ...
    struct Affiliate {
        ...
        uint totalRef;  // Total number of referrals
        address addr;   // Address of the affiliate
    }

    mapping(address => uint) public addressToAffiliate;
    mapping(string => uint) public affiliateIdToAffiliate;
    mapping(address => uint) public userToAffiliate;
...
// Modifiers
    ...
    function verifyAffiliate(string memory affiliateId) public onlyOwner validAffiliate(affiliateId) returns (bool) {
        affiliates[affiliateIdToAffiliate[affiliateId]].enabled = true;
        return true;
    }

    // Disable affiliate to refer users
    function disableAffiliate(string memory affiliateId) public onlyOwner validAffiliate(affiliateId) returns (bool) {
        affiliates[affiliateIdToAffiliate[affiliateId]].enabled = false;
        return true;
    }

    // Check if a affiliate is enabled
    function isAffiliateEnabled(string memory affiliateId) public view validAffiliate(affiliateId) returns (bool) {
    ...
    function withdrawAffiliate() public validAffiliateAddress {
        require(affiliates[addressToAffiliate[msg.sender]].enabled == false, "Affiliate is already enabled");
    ...
    function registerReferredUser(address userAddr, string memory affiliateId) internal validAffiliate(affiliateId) notZero(userAddr) {
        ...
        uint affiliateIdx = affiliateIdToAffiliate[affiliateId];
        require(affiliates[affiliateIdx].enabled == true, "Affiliate is not active");
    ...
    function getAffiliateAddress(address userAddr) external view returns(address) {
